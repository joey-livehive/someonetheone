...
export default function NoirStartPage() {
  ...
  const eta = useMemo(() => new Date(today.getTime() + 86400000), [today]);
    ...
    <main className="app">
      ...
      {phase === 'done' && (
        <div className="receipt screen-fade">
          {!guestUid && (
            <div style={{ padding: '14px 18px', marginBottom: 20, background: 'var(--highlight)', border: '1px solid var(--gold)', fontSize: 13, lineHeight: 1.6, color: 'var(--ink)' }}>
              네트워크 문제로 의뢰서가 정상 접수되지 않았을 수 있습니다.
              <br />
              잠시 후 다시 시도하시거나, 동일 번호로 재접수 부탁드립니다.
            </div>
          )}
          ...
          <div className="phase__footer">
            <Link href="/30" className="btn btn--dark btn--full btn--lg">
              홈으로 돌아가기
            </Link>
          </div>
        </div>
