...
|---|---|---|---|---|---|---|
| `owner` | internal | 게스트 매칭 | internal_adapter | internal_adapter | PAIR_FOR_OWNER | `/connection/casting/{uid}` |
| `owner` | insta | 인스타 매칭 | internal_adapter | FILTER_INSTA → PROFILE_INSTA | PAIR_FOR_OWNER | `/connection/casting/{uid}` |
| `partner` | internal | 소개받은 사람 (내부) | internal_adapter | internal_adapter | PAIR_FOR_PARTNER | `/connection/cast/{uid}` |
| `partner` | insta | 소개받은 사람 (외부) | internal_adapter | FILTER_INSTA → PROFILE_INSTA | PAIR_FOR_PARTNER | `/connection/cast/{uid}` |
...
type ConnectionReport = {
  ...
  axisNotes: AxisNote[]  | null   // 정확히 4개 (energy/judgment/sociability/action)
  meta: {
...
type AxisNote = {
  axis: 'energy' | 'judgment' | 'sociability' | 'action'
  narrative: string
}
```
...
**왜 alternate 인가** (v5 합의):
- owner × guest: 양쪽 다 설문 정량값 → 6축 radar 차트가 비교 가능 + 신뢰 가능
- owner × insta: partner 정량값이 추정값이라 차트가 오해 유발 → 4축 정성 narrative 가 더 정직
- → 두 표현은 같은 정보의 다른 채널. 동시에 채우면 위계가 모호하고 프론트가 어느 걸 lead 로 쓸지 결정 못 함.

**프론트 분기 패턴**:
```ts
if (report.radar) {
  renderRadarChart(report.radar)
} else if (report.axisNotes) {
  renderAxisNotes(report.axisNotes)
}
```
...
- **3a (push 완료, PR #95 머지)** — `ConnectionReport(perspective='owner')` 백엔드 조립
  - `build_owner_connection_report` (`darakbang-backend/.../connection_report_builder.py`)
  - partner.source=internal vs insta 분기 + lazy + cache hit/miss + PERSON × 2 + PAIR (옛 PAIR_SYSTEM_PROMPT)
  - sanity 스크립트로 internal / insta(mock) 양쪽 ConnectionReport JSON 검증 완료
- **3b** — 프론트 owner 페이지 실제 렌더 (`/connection/casting/{uid}`)
- **3c** — PAIR_FOR_OWNER 프롬프트 v2 + schema 위계 정리 (v5)
  - 인스타 합본 (`someonetheone/lib/casting/insta/system-prompt.ts`) 의 PERSON 부분 흡수 → 백엔드 PERSON 호출로
  - PAIR_FOR_OWNER 프롬프트로 옛 PAIR_SYSTEM_PROMPT 갈아치움 (placeholder padding 제거)
  - **schema 위계 정리 (v5)**: `PairContent.axisNotes` 제거 → `ConnectionReport.axisNotes` 로 끌어올림 (§ 7.3)
    - `radar` 와 `axisNotes` 둘 다 Optional, `partner.source` 에 따라 alternate 채움
    - 옛 4축 라벨 (`selfExpression/behavior`) → 표준 (`sociability/action`) 통일
    - PROFILE_INSTA Schema 버그 fix (`additionalProperties: True` 제거)
  - `PairContent.opening` 단일 필드명 적용 (이미 v4)
...
|---|---|---|---|---|
| 1 | owner | guest | `/connection/casting/{uid}` | PERSON × 2 + PAIR_FOR_OWNER 정상 출력 |
| 2 | owner | insta | `/connection/casting/{uid}` | FILTER_INSTA + PROFILE_INSTA eager 트리거 + 위 |
| 3 | partner | guest | `/connection/cast/{uid}` | PERSON × 2 + PAIR_FOR_PARTNER 정상 출력 |
| 4 | partner | insta | `/connection/cast/{uid}` | 위 |
...
- ✅ **(v5)** `radar` 와 `axisNotes` = alternate (둘 중 하나만 채움, `partner.source` 가 결정). `PairContent.axisNotes` 는 `ConnectionReport.axisNotes` 로 끌어올림. 3a sanity 결과에서 위계 모호함이 드러나 확정.
...
- **2026-05-11 v5 (PR 2 commit 3a sanity 후)**: `radar` 와 `axisNotes` 를 `ConnectionReport` top-level alternate 로 정리. `partner.source=internal` → `radar` 채움 (6축 차트), `partner.source=insta` → `axisNotes` 채움 (4축 narrative). 둘 다 Optional. `PairContent.axisNotes` 는 제거하고 `ConnectionReport.axisNotes` 로 끌어올림. PairContent 는 `opening + simulation` 만 남는 narrative-only. 3c scope 에 schema 위계 정리 + 4축 라벨 통일 + PROFILE_INSTA Schema 버그 fix 명시. 위계 모호함은 3a sanity 두 케이스 (internal vs insta) JSON 비교에서 드러남 — radar 와 axisNotes 가 같은 정보의 두 표현이라는 게 명확해짐.
