...
1. `casting_guests.profile_status` → `status` (CHANGE COLUMN, 값 분포 그대로)
2. `casting_external_profiles.filter_status` → `status` (CHANGE COLUMN, 값 분포 그대로)
3. `casting_profiles.profile_status` → `status` (CHANGE COLUMN) + `UPDATE ... SET status='active' WHERE status='ready'` (1119건)
4. `UPDATE casting_match_reports SET status='published' WHERE status='ready'` (761건)
5. `UPDATE casting_preview_reports SET status='published' WHERE status='ready'` (784건)
6. `ALTER TABLE casting_profiles ADD COLUMN profile_json LONGTEXT NULL`
